...
params = {
          'country'       : 'SO', #Name of country
          'IPC_csv'       : 'SO_Admin2_LHZ_2016_2_ML1.csv', #Name of csv from data team save in country folder if you do not want to specify full file path.
          'fsc_shapefile' : 'SO_Admin2_LHZ_2016_2.shp', #Shapefile name must not include a "." in the file name - only dot should be . before shp (i.e: ".shp").
          'outputfile'    : 'SO_IPC_Analysis_ML1.xlsx', #Name of file outputed by script
          'IPC_Aggregation': True, #Set to true is you want to aggregate data to a higher level and produce graphs 
          'IPC_Aggregation_Level':'LHZ' #Options are LHZ_Admin1 or LHZ or Admin1
          }

directory = os.path.join(os.getcwd(),params['country'])
os.chdir(directory)
    ...
    def sort_data(): #Sort dataframe values by LZCODE, Admin1, Admin2 if LH Zone Name is present. If shapefile is not a LH Zone intersect shapefile just sort by admin names
        if lhz == True:
            try:
                ...
                join.sort_values(by=['LZCODE','ADMIN1'],axis=0,inplace=True)
        else:
            try:
                join.sort_values(by=['ADMIN1','ADMIN2'],axis=0,inplace=True)
            except:
                join.sort_values(by=['ADMIN1'],axis=0,inplace=True)
        return join
...
class get_join_landscan_pop():
    def get_landscan():
        ...
        r = requests.get(landscan_url, verify=False)
...
class make_charts():
    ...
    def phase3_chart():
        fig, ax = plt.subplots()
        ax.bar(ipc_agg_df.index,ipc_agg_df['ipc_3plus_weight'], width=0.55)
        ax.set_ylabel("Percent in IPC 3 plus")
        ax.set_xlabel("Livelihood Zone")
        plt.title("Average Percent of Cycles in IPC Phase 3+")
        fig.tight_layout()
        plt.xticks(rotation=60)
        plt.savefig(params['IPC_Aggregation_Level'] + '_IPC_3plus.png',bbox_inches='tight')
        plt.show()
    def av_ipc_chart():
        fig, ax = plt.subplots()
        ax.bar(ipc_agg_df.index,ipc_agg_df['ipc_avg_weight'], width=0.55)
        ax.set_ylabel("IPC Area Phase")
        ax.set_xlabel("Livelihood Zone")
        ax.set_ylim((1,ipc_agg_df['ipc_avg_weight'].max()+0.2))
        plt.title("Average IPC Area Phase")
        fig.tight_layout()
        plt.xticks(rotation=60)
        plt.savefig(params['IPC_Aggregation_Level'] + '_IPC_Av.png',bbox_inches='tight')
        plt.show()

if params['IPC_Aggregation'] == True:
    ipc_agg_df = aggregate_data()
    print(ipc_agg_df.info())
    ipc_agg_df.to_csv('Aggregated_data.csv')
